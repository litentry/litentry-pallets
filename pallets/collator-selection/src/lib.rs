...
pub mod pallet {
	...
	/// Litentry: Reimplementation of block reward
	impl<T: Config + pallet_authorship::Config + pallet_transaction_payment_interface::Config>
		pallet_authorship::EventHandler<T::AccountId, T::BlockNumber> for Pallet<T>
	{
		fn note_author(author: T::AccountId) {
			let award = pallet_transaction_payment_interface::Pallet::<T>::fix_block_reward();
			// block reward is a fix amount.
			let PositiveImbalance = <T as Config>::Currency::deposit_creating(&author, award.into());
			Self::deposit_event(Event::<T>::BlockAuthorAwarded(award.into()));
