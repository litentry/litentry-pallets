...
// Litentry: Reimplemented
#[test]
fn authorship_event_handler() {
	new_test_ext().execute_with(|| {
		assert_ok!(TransactionPaymentInterface::set_fix_block_reward(Origin::root(), 15));
		...
		assert_eq!(Balances::free_balance(4), 100 - 10 + 15);

		// change block reward
		assert_ok!(TransactionPaymentInterface::set_fix_block_reward(Origin::root(), 35));
		Authorship::on_initialize(2);
		assert_eq!(Balances::free_balance(4), 100 - 10 + 15 + 35);
