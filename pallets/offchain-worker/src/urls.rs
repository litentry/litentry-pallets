...
pub const DATA_SOURCE_LIST: [DataSource; TOTAL_DATA_SOURCE_NUMBER as usize] =
	[DataSource::EthEtherScan, DataSource::EthInfura, DataSource::BtcBlockChain];
...
pub fn fetch_json_http_post<'a>(
	...
	let pending = http::Request::post(remote_url_str, vec![body])
		.send()
...
pub fn send_get_token() -> Result<Vec<u8>, &'static str> {
	let pending = http::Request::get(super::TOKEN_SERVER_URL)
		.send()
		.map_err(|_| "Error in sending http GET request")?;
...
pub fn parse_etherscan_balances(price_str: &str) -> Option<Vec<u128>> {
	...
	let result: Vec<u128> = token_info
		.result
		.iter()
		.map(|item| {
			match utils::chars_to_u128(&item.balance.iter().map(|i| *i as char).collect()) {
				Ok(balance) => balance,
				Err(_) => 0_u128,
			}
		})
...
pub fn parse_infura_balances(price_str: &str) -> Option<Vec<u128>> {
	...
	let result: Vec<u128> = token_info
		.iter()
		.map(|item| match utils::chars_to_u128(&item.result.iter().map(|i| *i as char).collect()) {
			Ok(balance) => balance,
			Err(_) => 0_u128,
		})
		.collect();
